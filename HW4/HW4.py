...
if __name__ == '__main__':
    ...
    if len(argv) > 1:
        files = (argv[1],)
        if len(argv) > 2:
            prob_cross = float(argv[2])
            if len(argv) > 3:
                prob_mutacion = float(argv[3])
                if len(argv) > 4:
                    MAX_ITER = int(argv[4])
                    if len(argv) > 5:
                        BCRC = bool(argv[5])
                        if len(argv) >6:
                            poblacion = int(argv[6])
    else:
        dir = "./P/"
    ...
    for file in files:
        ...
        handle.write("Instancia: " + str(file) + " | Función Objetivo: " + str(int(instancia.mejor_FO)) + " | Tiempo Computacional (s): " + str(t1_stop - t1_init) + "\n")
        for i, ruta in enumerate(instancia.mejor_solucion[0]):
            handle.write("Ruta #" + str(i+1)+ ": " + str(ruta)  + "\n")
        break
    handle.close()
